...
void cGame::initialise(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	for (int astro = 0; astro < 8; astro++)
	{
		...
		theAsteroids[astro]->setSpritePos({ astro * 150, 100});
...
void cGame::render(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	if (scoreIncrease)
	{
		gameTextList[1] = scoretoString.c_str();
		theTextureMgr->addTexture("Score", theFontMgr->getFont("spaceAge")->createTextTexture(theRenderer, gameTextList[1], SOLID, { 0, 255, 0, 255 }, { 0, 0, 0, 0 }));
		scoreIncrease = false;
	}
	...
	pos = { 100, 450, tempTextTexture->getTextureRect().w, tempTextTexture->getTextureRect().h };
	scale = { 1,1 };
	tempTextTexture->renderTexture(theRenderer, tempTextTexture->getTexture(), &tempTextTexture->getTextureRect(), &pos, scale);
	...
	while (SDL_PollEvent(&event))
	{
		...
		switch (event.type)
		{
			...
			case SDL_KEYDOWN:
				...
				case SDLK_RIGHT:
				{
					if (theRocket.getSpritePos().x > -1000)
					{
